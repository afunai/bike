...
* You can make nested apps inside an app (eg. blog comments inside a blog entry).
...
This is a quick tutorial for making a simple web application using Runo. All you need is a command line terminal and a few minutes.
...
In a command line terminal, go to an empty directory/folder where you want to build Runo projects.
Then type:

  runo init

This will create a "runo" directory/folder, which includes:
...
In the terminal, change to the newly created "runo" directory/folder.
Type:
...
There is some links to examples such like Blog, Contact Form, Forum, etc. Just see what can be done by plain HTML files. When you need to login, the initial account/password is root/root.

=== Step 5 - Create Your Own App

Let's call our app "fab".
It is a web-based forum where visitors can submit their own articles, and everybody can read them.
...
The content is: (You can just copy & paste, off course)
...
You have just created your first application. Open a bottle of your favorite drink and make yourself comfortable.
